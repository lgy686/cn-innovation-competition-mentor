...
- 不把项目材料无法支持的技术、专利、合同、客户、收入、奖项、实验数据或商业模式写成真实成果；核心技术章节可使用显著标注、待人工审核的模拟实验占位数据。
...
│   └── 产品照片、软件截图、应用现场图片
├── 参考文献/
│   ├── 核心技术相关 Markdown（同主题优先阅读）
│   └── 论文 PDF、Word、图片及其他技术资料
└── 优秀案例/
...
```text
使用 $cn-innovation-competition-mentor，读取当前项目“参考文献”目录中的全部文件，
优先阅读 Markdown，按项目口吻撰写核心技术章节，包含原理、LaTeX 公式、工作流程图和实验结果分析。
若缺少实验数据，可以生成明确标为待人工审核的模拟占位数据。
```

参考文献可作为项目采用或拟采用技术的写作依据，但不能单独证明项目已完成研发或取得真实性能结果。模拟数据会标注为 `【模拟占位数据，待人工审核，不代表已完成实验】`，审核替换前不得用于项目摘要、成果清单或对外定稿。
...
生成正式 Word 时，不应只靠逐段手动调字体；应复用官方模板样式，或使用项目专用的一级标题、二级标题、正文、图注、表注、表头、表正文和参考文献样式。赛事没有明确框架或格式时，按用户指定的优秀案例设置章节结构、篇幅比例和版式。商业部分定量数据必须采用可编辑的 Word 原生图表并插入相应论述位置：趋势用折线图，多类别比较用柱状图，构成用饼图/环形图，收入成本利润用柱线组合图。若当前环境无法可靠创建可编辑图表，会将其明确标为未完成项、保留数据表并说明推荐图表类型，而不会用静态截图冒充可编辑图表。
...
| 首次扫描、文件分类、参考文献读取顺序和项目接收流程 | `references/project-intake.md` |
...
| 商业计划书章节组织、案例框架、核心技术文献、公式、流程和实验写作 | `references/business-plan-writing.md` |
